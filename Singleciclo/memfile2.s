// memfile.s
// bruno.lorenzo.thomaz@gmail.com , marianatassan19@gmail.com , JULHO 2023
// Test ARM processor
// ADD, SUB, AND, ORR, LDR, STR, B
// TST, CMP, STRB, LDRB, BL, MOV
// If successful, it should write the value 26 to address 24
...
AROUND   
	...
	BEQ END             ; Se R4 = R2 pula para END 				
	ADD R2, R0, #13   	; shouldn't happen				
  	ADD R2, R0, #10		; shouldn't happen			  
END	STR R2, [R0, #100] 	; mem[100] = 7
	MOV R1, #15 		; R1 = 0xF	
    TST R1, #1          ; Verifica o ultimo bit de R1
    MOVEQ R6, #1		; se = ? R6 = 1
	MOVNE R6, #0        ; se = ? R6 = 0
    EOR R9, R2, R3      ; R9 = R2 xor R3 
    BL TESTE            ; Entra na função Teste 
    MOV R12, #0x1a      ; R12 = 0x1a
	B ENDLOOP           ; Pula para a função ENDLOOP
TESTE                   ; função TESTE
    MOV R6, #8          ; R6 = 8
	STRB R8, [R1, #7]   ; mem[22] = 0x000000FE
	LDR R10, [R1, #7]   ; R10 = mem[22] = 0x000000FE
	STR R8, [R1, #8]    ; mem[23] = 0xFFFFFFFE
    LDRB R11, [R1, #8]  ; R11 = mem[23] = 0x000000FE
    MOV R0, #0x16       ; R0 = #0x16
    MOV PC, LR          ; Retorna para instrução depois do BL           	
ENDLOOP                 ; Função ENDLOOP
    STR R12, [R1, #9]   ; mem[24] = 0x1a
